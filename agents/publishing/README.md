...
- Buzzsprout / Spotify-for-Podcasters API integration + RSS feed hosting
- LinkedIn API approval + OAuth 2.0 flow
- Twitter/X paid API tier
- Instagram Graph API + Facebook Business Account approval
...
│   └── mock_adapters.py       # Buzzsprout, LinkedIn, Twitter, Instagram
├── tests/
...
```bash
cd publishing_agent
python tests/test_publishing.py
```
...
PublishingAgent
    |
    +-- BuzzsproutAdapter   (podcasts → Spotify/Apple/Google)
    +-- LinkedInAdapter
    +-- TwitterAdapter
    +-- InstagramAdapter
```
...
```python
real_adapters = {
    Platform.LINKEDIN: RealLinkedInAdapter(oauth_client),
    Platform.BUZZSPROUT: BuzzsproutAdapter(api_key),
    # ... etc
}
...
```json
{
  ...
  "platform_post_id": "bzp_d328bd4fdd",
  "url": "https://mock-buzzsprout.com/episodes/bzp_d328bd4fdd",
