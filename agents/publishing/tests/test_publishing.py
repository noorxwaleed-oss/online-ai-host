...
from adapters.mock_adapters import MOCK_CONNECTIONS, AUDIT_LOG_PATH
...
def make_input(
    ...
    return PublishingInput(
        ...
        target_platforms=target_platforms or [Platform.BUZZSPROUT, Platform.LINKEDIN],
    )
...
def test_happy_path_two_platforms():
    ...
    out = agent.publish(make_input(target_platforms=[Platform.BUZZSPROUT, Platform.LINKEDIN]))
...
def test_all_four_platforms():
    ...
    out = agent.publish(make_input(target_platforms=[
        Platform.BUZZSPROUT, Platform.LINKEDIN,
...
def test_unconnected_platform_is_skipped():
    """User without LinkedIn linked should get SKIPPED_NOT_CONNECTED."""
    cleanup_audit_log()
    # Add a user with only Buzzsprout linked
    MOCK_CONNECTIONS["partial_user"] = {Platform.BUZZSPROUT}

    agent = PublishingAgent()
    out = agent.publish(make_input(
        user_id="partial_user",
        target_platforms=[Platform.BUZZSPROUT, Platform.LINKEDIN],
    ))
    ...
    skipped_result = next(r for r in out.results if r.platform == Platform.LINKEDIN)
    assert skipped_result.status == PublishStatus.SKIPPED_NOT_CONNECTED
    assert "linkedin" in skipped_result.error_message.lower()
...
def test_audit_log_records_publishes():
    ...
    cleanup_audit_log()
    agent = PublishingAgent()
    agent.publish(make_input(target_platforms=[Platform.BUZZSPROUT]))
    agent.publish(make_input(target_platforms=[Platform.LINKEDIN]))
    ...
    assert record1["platform"] == "buzzsprout"
    assert record2["platform"] == "linkedin"
...
def test_target_platforms_must_have_no_duplicates():
    ...
    try:
        PublishingInput(
            ...
            target_platforms=[Platform.BUZZSPROUT, Platform.BUZZSPROUT],
        )
...
def test_linkedin_description_length_limit():
    """LinkedIn adapter rejects descriptions over 1300 chars."""
    cleanup_audit_log()
    audio, cover = make_temp_files()
    ...
    assert "1300" in out.results[0].error_message
...
def test_unique_post_ids_across_publishes():
    ...
    cleanup_audit_log()
    agent = PublishingAgent()
    out1 = agent.publish(make_input(target_platforms=[Platform.BUZZSPROUT]))
    out2 = agent.publish(make_input(target_platforms=[Platform.BUZZSPROUT]))
...
def test_overall_status_is_success_only_when_all_succeed():
    ...
    inp = PublishingInput(
        ...
        user=UserContext(user_id="demo_user"),
        target_platforms=[Platform.BUZZSPROUT],
    )
    agent = PublishingAgent()
    out = agent.publish(inp)
    assert out.overall_status == PublishStatus.FAILED
...
if __name__ == "__main__":
    ...
    tests = [
        ...
        test_linkedin_description_length_limit,
