...
def get_feed_url(user_id: str) -> str | None:
    ...
    url = f"https://res.cloudinary.com/{CLOUD_NAME}/raw/upload/users/{user_id}/feed_info.json"
    try:
...
def get_project_metadata(user_id: str, project_id: str) -> dict:
    ...
    url = f"https://res.cloudinary.com/{CLOUD_NAME}/raw/upload/users/{user_id}/projects/{project_id}/metadata.json"
    try:
        response = requests.get(url)
